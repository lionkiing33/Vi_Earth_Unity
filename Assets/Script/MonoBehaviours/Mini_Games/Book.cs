...
public class Book : MonoBehaviour {
    ...
    //해당 미션 오브젝트
    public InteractObject book;

    public RectTransform BookPanel;
    public Sprite first_page;
    public Sprite last_page;
    ...
    void Start()
    {
        ...
        library = this.transform.parent.transform.parent.transform.parent.gameObject;
        mini_game = library.transform.GetChild(0).gameObject.GetComponent<Mini_Game>();
    ...
    void Update()
    {
        if (LeftNext.sprite == bookPages[TotalPageCount - 1] && currentPage == TotalPageCount)
        {
            Exit_Panel();
        }
        else if (pageDragging&&interactable)
        {
            UpdateBook();
        }
    ...
    public void DragRightPageToPoint(Vector3 point)
    {
        ...
        Left.sprite = (currentPage < bookPages.Length) ? bookPages[currentPage] : first_page;
        Left.transform.SetAsFirstSibling();
        
        Right.gameObject.SetActive(true);
        Right.transform.position = RightNext.transform.position;
        Right.transform.eulerAngles = new Vector3(0, 0, 0);
        Right.sprite = (currentPage < bookPages.Length - 1) ? bookPages[currentPage + 1] : last_page;

        RightNext.sprite = (currentPage < bookPages.Length - 2) ? bookPages[currentPage + 2] : last_page;
    ...
    public void DragLeftPageToPoint(Vector3 point)
    {
        ...
        Left.transform.eulerAngles = new Vector3(0, 0, 0);
        Left.sprite = (currentPage >= 2) ? bookPages[currentPage - 2] : first_page;

        LeftNext.sprite = (currentPage >= 3) ? bookPages[currentPage - 3] : first_page;
    ...
    void UpdateSprites()
    {
        LeftNext.sprite= (currentPage > 0 && currentPage <= bookPages.Length) ? bookPages[currentPage-1] : first_page;
        RightNext.sprite=(currentPage>=0 &&currentPage<bookPages.Length) ? bookPages[currentPage] : last_page;
    }
    ...
    private void Exit_Panel()
    {
        ...
        mini_game.ClearMiniGame(book);
        if (book.min < book.max)
        {
            currentPage = 0;
            LeftNext.sprite = first_page;
