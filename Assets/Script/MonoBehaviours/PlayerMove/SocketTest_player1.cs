...
public class SocketTest_player1 : MonoBehaviour
{
    private float x, y = -8;
    ...
    void Start()
    {
        SocketManger.Socket.On("sendTomove1", (data) => {
            string json = JsonConvert.SerializeObject(data.Json.args[0]);
            Player1Move response = JsonUtility.FromJson<Player1Move>(json);
            x = response.x;
            y = response.y;
			check = true;
        });
